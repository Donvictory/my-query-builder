...
// Lightweight, secure regex highlighter for MongoDB JSON syntax
function highlightMongo(mongo: string): string {
    ...
    // Highlight MongoDB operational keywords ($and, $or, $regex, $gt, etc.)
...
export function QueryPreview() {
    ...
    return (
        <div className="border border-border/70 rounded-xl bg-card overflow-hidden shadow-xs">
            ...
            <div className="relative bg-slate-950 dark:bg-slate-950 p-4 font-mono text-xs select-text">
                ...
                <button
                    onClick={handleCopy}
                    className={`absolute right-3.5 top-3.5 h-7 px-2.5 rounded-lg border text-[10px] font-bold flex items-center gap-1.5 transition-all shadow-sm ${
                        copied
                ...
                <pre className="text-slate-100 font-mono text-[11px] leading-relaxed overflow-x-auto max-h-64 whitespace-pre-wrap break-all custom-scrollbar pb-1 pr-16 select-text selection:bg-cyan-500/30">
                    <code 
